...
test("the localStorage Spec example creates the correct file & contents", async () => {
  const expectedFilePath = await getFilePath(
    resolve(
      __dirname,
      "../../../../node_modules/cli-testing-library/dist/cypress/integration/local-storage.spec.js"
    )
  );

  const template = fs.readFileSync(
    resolve(__dirname, "../../src/lib/templates/examples/localStorage.js"),
    "utf8"
  );
  ...
  expect(await findByText("localStorage Spec")).toBeInTheConsole();
  userEvent.keyboard("[ArrowDown]");
  userEvent.keyboard("[ArrowDown]");
  userEvent.keyboard("[ArrowDown]");
  userEvent.keyboard("[Enter]");
  ...
  userEvent.keyboard("localStorage");
  userEvent.keyboard("[Enter]");
